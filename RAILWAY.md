...
4. **Verify Project Detection:**
   - Railway should auto-detect Next.js using Railpack
   - If not detected, ensure `package.json` is in root directory
   - Railway will use Railpack builder automatically
...
**Solution:** Railway's Railpack should automatically include build tools for native modules. If issues persist:

- Ensure Railway is using Railpack builder (auto-detected for Next.js)
- Check build logs for specific error messages
- Railpack should automatically install python3, make, and gcc for native modules
