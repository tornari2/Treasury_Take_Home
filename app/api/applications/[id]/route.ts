  ...
  try {
    ...
    const parsedApplication = {
      ...
      label_images: labelImages.map((img) => ({
        ...
        extracted_data: img.extracted_data
          ? JSON.parse(img.extracted_data)
          : null,
        verification_result: img.verification_result
          ? JSON.parse(img.verification_result)
          : null,
      })),
    };
...
export async function PATCH(
  ...
  try {
    ...
    if (status && !["pending", "approved", "rejected"].includes(status)) {
      ...
      if (status === "needs_review") {
        return NextResponse.json(
          {
            error:
              "needs_review status is no longer supported. Use pending instead.",
          },
          { status: 400 },
        );
      }
    ...
    // Update application
    if (status) {
      applicationHelpers.updateStatus(
        applicationId,
        status,
        review_notes || null,
      );
  ...
  try {
    ...
    if (applicationData.producerName.trim() !== application.applicant_name) {
      const updateNameStmt = db.prepare(
        "UPDATE applications SET applicant_name = ? WHERE id = ?",
      );
    ...
    if (applicationData.beverageType !== application.beverage_type) {
      const updateTypeStmt = db.prepare(
        "UPDATE applications SET beverage_type = ? WHERE id = ?",
      );
    ...
    if (imageFiles.length > 0) {
      ...
      const existingImages =
        labelImageHelpers.findByApplicationId(applicationId);
      ...
      const updatedApplicationDataString = JSON.stringify(
