...
export default function ReviewPage() {
  ...
  const [fetchError, setFetchError] = useState<"not_found" | "error" | null>(
    null,
  );
  ...
  const [batchApplications, setBatchApplications] = useState<number[] | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (batchParam === "true" || storedBatchApps) {
      try {
        ...
        if (appIds && Array.isArray(appIds) && appIds.length > 0) {
          ...
          if (index !== -1) {
            ...
            setCurrentBatchIndex(
              storedBatchIndex ? Number(storedBatchIndex) : 0,
            );
            setIsInBatch(true);
          }
        ...
        console.error(
          "Error parsing batch applications from sessionStorage:",
          error,
        );
        // Clear invalid data
        sessionStorage.removeItem("batchApplications");
  ...
  useEffect(() => {
    ...
    if (
      application.label_images.length > 0 &&
      (shouldVerify || !hasVerification)
    ) {
          ...
          if (storedBatchApps) {
            try {
              ...
              if (Array.isArray(appIds) && appIds.length > 0) {
                ...
                const updatedBatch = appIds.filter(
                  (id: number) => id !== Number(fetchId),
                );
                if (updatedBatch.length > 0) {
                  ...
                  const storedIndex =
                    sessionStorage.getItem("batchCurrentIndex");
                  const currentIdx = storedIndex ? Number(storedIndex) : 0;
                  const nextIndex =
                    currentIdx < updatedBatch.length
                      ? currentIdx
                      : updatedBatch.length - 1;
  ...
  const triggerVerification = async () => {
    ...
    try {
      ...
      if (error instanceof DOMException && error.name === "AbortError") {
        alert(
          "Verification timed out. The service may be busy. Please try again in a moment.",
        );
      } else if (
      ...
      if (index !== -1) {
        ...
        return (
          <>
            {before}
            {shouldBold ? (
              <span className="font-bold">{govWarning}</span>
            ) : (
              govWarning
            )}
            {after}
  ...
  const firstImageVerificationResult =
    currentApp.label_images[0]?.verification_result;
              ...
              {isInBatch && batchApplications && currentBatchIndex !== null && (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-white font-medium px-2">
                    Application {currentBatchIndex + 1} of{" "}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <AlertDescription>
                              ...
                              <div className="text-sm text-yellow-800 space-y-1">
                                {qualityIssues.hasPerspectiveDistortion && (
                                  <div>
                                    • Weird angle/perspective distortion
                                    detected
                                  </div>
                                )}
                                ...
                                {qualityIssues.hasGlare && (
                                  <div>• Glare/hotspots detected</div>
                                )}
                                <div className="text-xs text-yellow-700 mt-2 italic">
                                  Image was automatically preprocessed to
                                  improve readability
                                </div>
                              </div>
                            </AlertDescription>
            ...
            <div className="bg-white rounded-lg shadow p-6">
              <h2 className="text-xl font-semibold mb-2">
                AI Verification Recommendations
              </h2>
              <p className="text-sm text-muted-foreground mb-4">
                These are AI-generated verification results to assist your
                review. Use your professional judgment to make the final
                decision.
              </p>
              ...
              {Object.keys(verificationResult).length === 0 || verifying ? (
                  ...
                  {!verifying && (
                    <>
                      <div className="text-muted-foreground">
                        No verification results yet. Click &quot;Verify&quot; to
                        process.
                      </div>
                      <Button
                ...
                <div className="space-y-3">
                  {Object.entries(verificationResult).map(
                    ([fieldName, result]: [string, any]) => {
                      const isSoftMismatch = result.type === "soft_mismatch";
                      return (
                        <Alert
                          key={fieldName}
                          variant={
                            getFieldStatusVariant(result) === "destructive"
                              ? "destructive"
                              : "default"
                          }
                            ...
                            <div className="flex-1">
                              ...
                              {(() => {
                                ...
                                const isPresenceField =
                                  result.rule &&
                                  result.rule.startsWith("PRESENCE:");
                                const isPresenceNotFound =
                                  isPresenceField &&
                                    ...
                                    <>
                                      {(() => {
                                          ...
                                          return (
                                            <div className="text-sm mt-1 text-foreground">
                                              ...
                                              <span className="text-foreground">
                                                A Class/Type designation is
                                                required whenever a Varietal is
                                        ...
                                        if (
                                          ...
                                          const beverageType =
                                            currentApp?.beverage_type?.toLowerCase();
                                        ...
                                        if (
                                          ...
                                          return (
                                            <div className="text-sm mt-1 text-foreground">
                                              ...
                                              <span className="text-foreground">
                                                Must appear if the product has
                                                10 ppm or more (total) sulfur
                                                dioxide.
                                              </span>
                                            </div>
                                          );
                                        ...
                                        if (isAgeStatement && result.expected) {
                                          ...
                                          if (
                                            result.expected.startsWith("N/A")
                                          ) {
                                            return (
                                              <div className="text-sm mt-1 text-muted-foreground">
                                                {result.expected}
                ...
                <div className="space-y-2">
                  <Label htmlFor="review-notes">
                    ...
                    <span className="text-muted-foreground font-normal ml-2">
                      (Recommended, especially when overriding verification
                      results)
                    </span>
                  </Label>
                  <Textarea
