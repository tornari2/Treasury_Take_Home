...
- **Government Warning Bold Check:**
  ...
  - Validation checks `formatChecks.governmentWarningBold === false` to trigger HARD_MISMATCH if not bold
  - Previously formatChecks were hardcoded to null, so the check never ran

- **FormatChecks Extraction Pipeline:**
  - `lib/openai-service.ts`: Extracts `formatChecks` from OpenAI JSON response
  - Returns `formatChecks` object including: `governmentWarningAllCaps`, `governmentWarningBold`, `remainderBold`, `surgeonCapitalized`, `generalCapitalized`
  - `lib/verification.ts`: Accepts `formatChecks` parameter and passes to validation
  - `app/api/applications/[id]/verify/route.ts`: Extracts and passes `formatChecks` to `verifyApplication`
  - `lib/batch-processor.ts`: Extracts and passes `formatChecks` to `verifyApplication`

- **Review Page Display:**
  - Updated to use `formatChecks.governmentWarningBold` for health warning display
  - Falls back to checking validation result rule if formatChecks not available
  - Final fallback: checks if "GOVERNMENT WARNING" text is present
...
- **NOT_FOUND Fields Display:**
  - Fields with expected presence (like Alcohol Content, Net Contents, Class/Type) now show only "Field not found" without Expected/Extracted labels
  - Changed "Field not found" text color to grey (text-muted-foreground) for better visual consistency
  - Simplified display for presence-based fields that are missing
