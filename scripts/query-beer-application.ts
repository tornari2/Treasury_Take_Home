...
const applicationData = applicationDataField ? JSON.parse(applicationDataField) : null;
...
if (applicationData) {
  ...
  if (applicationData.producerAddress) {
    ...
    console.log(`    State: ${applicationData.producerAddress.state || 'null'}`);
  } else {
  ...
  console.log(`  Label Images: ${applicationData.labelImages ? applicationData.labelImages.length : 0} image(s)`);
...
if (labelImages.length > 0) {
  labelImages.forEach((img, idx) => {
    ...
    console.log(`  Has Verification Result: ${img.verification_result ? 'Yes' : 'No'}`);
